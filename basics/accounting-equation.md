...
- 損益の等式（P/L の骨格）: 収益 − 費用 = 利益（純資産の増減）

この 2 つが連動することで、複式簿記の「左右一致」と、期末利益が純資産へ取り込まれる仕組みが成立します。
...
期末に P/L の結果（利益）が B/S の純資産へ繰り入れられ、翌期の期首残高に反映されます。
...
- B/S の整合: 資産 = 負債 + 純資産を常に確認（試算表でチェック）
- P/L と B/S の連動: 期間損益が翌期の純資産へ繰入れているか確認

## 関連リンク

- 五要素と借方貸方: [簿記の基本構造：5 つの勘定科目グループ](../daily-activities/account.md)
- 仕訳の基礎と三分法: [仕訳の基礎と典型パターン](./journal-entries.md)
- 月次試算表と検算: [月次試算表の作り方と検算手順](../what-to-do-every-month/trial-balance.md)
- B/S と P/L の関係: [貸借対照表と損益計算書の違い](../daily-activities/difference-between-balance-sheet-and-income-statement.md)
